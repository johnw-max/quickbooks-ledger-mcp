...
2026-08-19 起本机无法 SSH 到部署主机（见文末"网络现状"）。Hetzner 云控制台的
浏览器终端走 443，不受影响，可以在那里执行：

```bash
cd <REPO> && sudo bash deploy/redeploy-0.6.0.sh
```

脚本会拉取正确提交、构建镜像、在现役容器**旁边**起候选、等健康、逐项核验候选
确实是修复后的构建（版本、18 工具、迁移头 035、ready），全部通过才移动 nginx，
最后从外部复核。任一步不符就中止且不改动任何东西。现役容器全程不停。

下面是同样的步骤拆开写，供手动执行或排查时对照。
...
本机到部署主机的 22 端口不通，因此这一轮无法由本机直接部署。诊断记录：

- 走 VPN（`utun4` 持有默认路由）：22 端口建立后立刻 EOF，按 IP 和按域名一致，
  是中间设备拒绝的特征。
- 用 `IP_BOUND_IF` 绑到 `en0` 绕开隧道：22 端口静默超时。
- 同一条 `en0` 路径：2222 / 22222 / 2200 / 8022 / 2022 / 62222 全部超时。
- 同一条 `en0` 路径：80 与 443 正常。

两条路径都只剩 web 端口，所以这是出网或主机防火墙的变化，不是 ssh 配置问题。
该主机在 2026-08-13 还接受过本机的 SSH（`known_hosts` 当天写入三条记录），
说明是这六天内发生的改动。

`~/.ssh/config` 里已备好该主机的配置（密钥、经本地代理的 HTTP CONNECT 隧道、
以及 `~/.ssh/http-connect-proxy.py`，因为 macOS 自带 `nc -X connect` 会把代理
返回的 `HTTP/1.1 200` 误判为错误）。22 端口一旦恢复可达，这些无需改动即可直接用。
