...
此处原本写着「代码路径已就绪，只是沙盒验不了」。**那句话是错的**，而且错在它是从
「代码在」推断出来的，不是跑出来的——和更早那次「生产跑的是修复前的构建」是同一类
错误，一周内复发了一次。实跑之后它不但没通过，还带出三个此前无任何验收条目覆盖的缺陷。

在 Sandbox Company US c694 上开多币种、币种表加入 SGD 之后，完整链路已跑通：
Bill **154**，`MBC-2026-0820`，S$1,635.00，汇率 0.783503，发出与回读逐字段一致，
QuickBooks 界面独立确认，provider 写入恰好 +1，且没有重复单据。

跑通它需要的不只是「加个汇率字段」：

- **联系人币种**。QuickBooks 的联系人 `CurrencyRef` 建档即锁死、交易币种必须与之一致，
  而连接器建联系人时从不写它。两阶段暂存又必然先建联系人后建单据，所以多币种账套下
  外币单据**永远**入不了账。改为由编译器从引用该联系人的单据推导，不新增字段。
- **dispatch 之后的三态**。Intuit 完成响应并以 Fault 拒绝，是「确认未写入」的证据；
  原实现把它塌陷进「未知」，而单据 request id 是内容哈希、修好环境从不改变它，
  于是一次拒绝让单据永久入不了账。
- **换代**。「确认未写入」要真的可用，只能在同一逻辑操作上开新一代（`.gN` + 全新
  Intuit 幂等键），且只在有证据证明从未写入时才允许。
- **未知的出口**。真正的未知只能由人去 QuickBooks 查，`quickbooks_resolve_unknown_write`
  记录其判定；ABSENT 方向由服务端按自然键检索否决错误判定。

### 仍未验证

**操作者身份**。存证记录 `attestationAuthority: HUMAN_EXPLICIT_CONFIRMATION`，但
传输主体是 OAuth 安装态，agent 与它代表的人呈现同一个 `actorId`，所有 broker 签发的
token 都是 `INSTALLATION_ONLY`。确认句把存证绑定到某一行、并使其成为一次刻意动作，
但它不构成身份认证。ABSENT 方向的安全性因此**不建立在人身上**，而建立在服务端的
自然键否决检索上。收据如实记录了 assurance 级别，等到有 token 携带
`TRUSTED_HOST_CONTEXT` 时，加一行硬闸即可。

**重放的对象是否仍在账上**。已修：重放终态成功前先确认对象仍存在（仅 CREATE，
只验存在不验字段相等，只有 NOT_FOUND 算证据）。但这条只有单元测试覆盖，
未在真实账套上删一张单再重入验证过。
