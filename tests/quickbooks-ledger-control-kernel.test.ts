...
const base = {
  ...
  standingDelegations: [{
    delegationId: "delegation-1", revision: 1, status: "ACTIVE" as const, providerId: "quickbooks",
...
describe("QuickBooks embedded Ledger Control Kernel", () => {
  ...
  it.each([
    ...
    ["wrong target", { standingDelegations: [{ ...base.standingDelegations[0], tenantIds: ["999"] }] }, "STANDING_DELEGATION_TARGET_MISMATCH"],
    ["expired target", { target: { ...base.target, targetSessionExpiresAt: now } }, "TARGET_SESSION_EXPIRED"],
